...
function Opcodes.isAux(op)
    ...
    return info and info.aux or false
end

function Opcodes.getOpLength(op)
    local info = Opcodes.list[op]
    if not info then return 1 end
    return info.aux and 2 or 1
end

function Opcodes.isJump(op)
    local name = Opcodes.list[op] and Opcodes.list[op].name
    if not name then return false end
    
    
    return name == "JUMP" or name == "JUMPBACK" or name == "JUMPIF" 
        or name == "JUMPIFNOT" or name == "JUMPIFEQ" or name == "JUMPIFLE"
        or name == "JUMPIFLT" or name == "JUMPIFNOTEQ" or name == "JUMPIFNOTLE"
        or name == "JUMPIFNOTLT" or name == "FORNPREP" or name == "FORNLOOP"
        or name == "FORGPREP" or name == "FORGLOOP" or name == "FORGPREP_INEXT"
        or name == "FORGPREP_NEXT" or name == "JUMPXEQKNIL" or name == "JUMPXEQKB"
        or name == "JUMPXEQKN" or name == "JUMPXEQKS" or name == "CMPPROTO"
end

function Opcodes.isFastCall(op)
    local name = Opcodes.list[op] and Opcodes.list[op].name
    if not name then return false end
    return name == "FASTCALL" or name == "FASTCALL1" or name == "FASTCALL2"
        or name == "FASTCALL2K" or name == "FASTCALL3" or name == "FASTPCALL"
end

function Opcodes.isLoopJump(op)
    local name = Opcodes.list[op] and Opcodes.list[op].name
    if not name then return false end
    return name == "JUMPBACK" or name == "FORGLOOP" or name == "FORNLOOP"
end

function Opcodes.isFallthrough(op)
    local name = Opcodes.list[op] and Opcodes.list[op].name
    if not name then return true end
    return name ~= "RETURN" and name ~= "JUMP" and name ~= "JUMPBACK" and name ~= "JUMPX"
end
